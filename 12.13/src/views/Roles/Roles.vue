<template>
  <div class="roles">
      ...
      <el-table-column label="操作" width="400">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="danger" icon="el-icon-delete" @click="del(scope.row.id, scope.$index)"
            >删除</el-button
          >
          <el-button size="mini" type="warning" icon="el-icon-setting" @click="addTree(scope.row)">分配权限</el-button>
        </template>
      </el-table-column>
    </el-table>
		<!-- 添加角色 -->
    ...
    <!-- 分配权限 -->
    <RolesTree ref="TreeRoles" @Setrole="Setrole" />
  </div>
</template>
<script>
import AddrolesVue from "./Addroles.vue"
import RolesTree from "./RolesTree.vue"
