            ...
            <el-table-column type="expand">
              <template slot-scope="scope">
                <el-button
                  size="mini"
                  type="primary"
                  v-for="(item, i) in scope.row.cat_name"
                  :key="i"
                  closable
...
export default {
  ...
  methods: {
    //   获取所有的商品分类列表
    async getCateList() {
      const { data: res } = await this.$http.request("/categories/:id/attributes");
      console.log(res);
    ...
    async getParamsData() {
      ...
      const { data: res } = await this.$http.request(`categories/${this.getCateId}/attributes`, {
    ...
    addParams() {
      this.$refs.addFromRef.validate(async (valid) => {
        if (!valid) return;
        const { data: res } = await this.$http.request("post", `categories/${this.getCateId}/attributes`, {
    ...
    async showEditDialog(attrId) {
      const { data: res } = await this.$http.request(`categories/${this.getCateId}/attributes/${attrId}`, {
    ...
    async removeParams(attrId) {
      ...
      const { data: res } = await this.$http.request("delete", `categories/${this.getCateId}/attributes/${attrId}`);
      if (res.meta.status !== 200) {
    ...
    async saveAttrVals(row) {
      const { data: res } = await this.$http.request("put", `categories/${this.getCateId}/attributes/${row.attr_id}`, {
