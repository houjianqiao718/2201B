<template>
  ...
  <div class="setup">
    ...
    <div class="role" v-show="flag == 0">
      ...
      <!-- 表格 -->
      <el-table :data="$store.state.Setup.list" style="width: 100%" border>
        ...
        <el-table-column fixed="right" label="操作" width="300">
          <template slot-scope="scope">
            ...
            <el-button link type="danger" size="mini" @click="del(scope.row.id)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页 -->
...
<script>
import { Setupsee, AddSetup, Addrole, AddroleOk, Editrole, Delrole } from "@/api/Setup"
export default {
  data() {
    return {
      flag: 0, //切换状态
  ...
  methods: {
    getSetup() {
      /*请求数据*/
      this.$store.dispatch("Setup/getList", this.params)
    },
    // 公司信息数据
    getSetupsee() {
      Setupsee().then((res) => {
