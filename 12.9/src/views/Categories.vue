    ...
    <el-card>
      ...
      >
        ...
        <template slot="order" slot-scope="scope">
          ...
          <el-button size="mini" type="success" v-else-if="scope.row.cat_level === 1">二级</el-button>
...
export default {
  ...
  methods: {
    // 获取商品分类
    async getCateList() {
      const { data: res } = await this.$http.request("/categories", {
      ...
      this.cateList_ = JSON.stringify(res.data.result);
      this.pages();
    ...
    // 分页逻辑
    pages() {
      let arr = JSON.parse(this.cateList_);
      this.cateList = arr.slice(
